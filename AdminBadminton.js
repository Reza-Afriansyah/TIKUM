...
// Data lapangan dan jadwal khusus BADMINTON - HARGA DIPERBAIKI
const facilities = {
    badminton: {
        name: "Badminton",
        fields: [
        ...
        // Harga khusus badminton - DIPERBAIKI
        timePrices: {
            "07:00 - 08:00": 45000,
            "08:00 - 09:00": 45000,
            "09:00 - 10:00": 45000,
...
// Handle booking form submission - DIPERBAIKI UNTUK WAKTU 23:00-00:00
function handleBooking(e) {
    ...
    if (paidAmount > totalAmount) {
        alert('❌ Jumlah pembayaran tidak boleh lebih dari total biaya!');
        return;
    }
    ...
    if (bookingStart < now) {
        alert('❌ Tidak bisa booking di waktu yang sudah lewat! Silakan pilih waktu yang akan datang.');
        return;
    }
    ...
    if (!isAvailable) {
        alert('❌ Lapangan sudah dipesan pada waktu tersebut! Silakan pilih waktu lain.');
        return;
    }
    ...
    let successMessage = '✅ Booking Badminton berhasil disimpan!\n\n';
    successMessage += `📅 ${facilities.badminton.name} - ${facilities.badminton.fields.find(f => f.id == fieldNumber).name}\n`;
    ...
    if (customerType === 'member') {
        successMessage += `💰 Paket Member: Rp ${totalAmount.toLocaleString()} per bulan\n`;
        successMessage += `⏱️ 4x sesi per bulan, 3 jam per sesi (total 12 jam)\n`;
    } else {
...
// Fungsi untuk menghitung total biaya berdasarkan tipe pelanggan
function calculateTotalAmount(customerType, bookingTime = null) {
    if (customerType === 'member') {
        // Member: Rp 425.000 per bulan (paket)
        return 425000;
    } else {
...
// Update default amount based on selected field, waktu, tanggal, dan tipe pelanggan
function updateDefaultAmount() {
    ...
    if (fieldId) {
        ...
        const price = calculateTotalAmount(customerType, bookingTime);
...
function createMemberInfo() {
    ...
    memberInfo.innerHTML = `
        ...
        • Biaya: Rp 425.000 per bulan<br>
        • 4x sesi per bulan<br>
        • 3 jam per sesi<br>
        • Total 12 jam per bulan
    `;
    ...
    if (bookingDate && fieldId) {
        ...
        facilities.badminton.timeSlots.forEach(slot => {
            ...
            if (!isPastTime && !isBooked) {
                ...
                const price = getDynamicPrice(slot, bookingDate, customerType);
                
                if (customerType === 'member') {
                    option.textContent = `${slot} - Paket Member`;
                } else {
        ...
        if (availableSlots === 0) {
            ...
            if (customerType === 'member') {
                infoOption.textContent = `✅ ${availableSlots} waktu tersedia untuk Member - Pilih waktu di atas`;
            } else {
                ...
                <div class="management-details">
                    ...
                    ${reservation.customerType === 'member' ? `
                        <div style="background: #dbeafe; padding: 8px; border-radius: 5px; margin-top: 10px; font-size: 0.9rem;">
                            <strong>💎 Paket Member:</strong> 4x sesi per bulan, 3 jam per sesi
                        </div>
                    ` : ''}
                </div>
            </div>
...
function handleEdit(e) {
    ...
    if (reservationIndex !== -1) {
        ...
        alert('Booking badminton berhasil diperbarui!');
    }
}
...
function deleteBooking(bookingId) {
    if (confirm('Apakah Anda yakin ingin menghapus booking badminton ini?')) {
        ...
        alert('Booking badminton berhasil dihapus!');
    }
}
...
console.log('✅ Admin Badminton loaded successfully with fixed date icons and new table layout!');