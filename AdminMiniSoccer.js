...
function getDynamicPrice(timeSlot, bookingDate, customerType = 'umum') {
    ...
    // Jika member, berikan diskon Rp 50.000 langsung
    if (customerType === 'member') {
        return Math.max(basePrice - 50000, 0); // Diskon Rp 50.000, pastikan tidak negatif
    }
...
// Fungsi untuk menghitung total biaya berdasarkan tipe pelanggan - DIPERBAIKI
function calculateTotalAmount(customerType, bookingTime = null, bookingDate = null) {
    if (customerType === 'member') {
        // Member: Rp 1.500.000 per bulan (paket)
        return 1500000;
    } else {
...
function createMemberInfo() {
    ...
    memberInfo.innerHTML = `
        ...
        • Biaya: Rp 1.500.000 per bulan<br>
        • 4x sesi per bulan<br>
        • 2 jam per sesi<br>
        • Total 8 jam per bulan
    `;
...
function handleBooking(e) {
    ...
    if (customerType === 'member') {
        ...
        successMessage += `⏱️ 4x sesi per bulan, 2 jam per sesi (total 8 jam)\n`;
    } else {
                ...
                <div class="management-details">
                    ...
                    ${reservation.customerType === 'member' ? `
                        <div style="background: #dbeafe; padding: 8px; border-radius: 5px; margin-top: 10px; font-size: 0.9rem;">
                            <strong>💎 Paket Member:</strong> 4x sesi per bulan, 2 jam per sesi
                        </div>
                    ` : ''}
                </div>
            </div>
...
function handleEdit(e) {
    ...
    if (reservationIndex !== -1) {
        ...
        alert('Booking mini soccer berhasil diperbarui!');
    }
}
...
function deleteBooking(bookingId) {
    if (confirm('Apakah Anda yakin ingin menghapus booking mini soccer ini?')) {
        ...
        alert('Booking mini soccer berhasil dihapus!');
    }
}
...
console.log('✅ Admin Mini Soccer loaded successfully with fixed date icons and new table layout!');